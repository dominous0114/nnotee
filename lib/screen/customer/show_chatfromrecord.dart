...
class ShowChatFromRecord extends StatefulWidget {
  final CustomerRecordModel customerRecordModel;
  const ShowChatFromRecord({Key key, this.customerRecordModel})
      : super(key: key);

  @override
  _ShowChatFromRecordState createState() => _ShowChatFromRecordState();
}
...
class _ShowChatFromRecordState extends State<ShowChatFromRecord> {
  @override
  void initState() {
    customerRecordModel = widget.customerRecordModel;
    checkUser();
    super.initState();
  }
  ...
  Future<Null> addChatThread() async {
    String url =
        '${MyConstant().domain}/mobile/addChat.php?isAdd=true&idCus=$idCus&idStore=${customerRecordModel.id}';
    try {
  ...
  Future<Null> addChatDetailThread() async {
    String url =
        '${MyConstant().domain}/mobile/addChatDetail.php?isAdd=true&user_from=$usernameCus&user_to=${customerRecordModel.username}&content=$content&chatId=$pro';
    try {
      Response response = await Dio().get(url);
      print('res = $response');
      if (response.toString() == 'true') {
  ...
  Future<List<dynamic>> readDataChat(int value) async {
    ...
    String url =
        '${MyConstant().domain}/mobile/getChatDetail.php?isAdd=true&userCus=$usernameCus&userStore=${customerRecordModel.username}';
    await Dio().get(url).then((value) {
  ...
  Future<Null> checkUser() async {
    ...
    String url =
        '${MyConstant().domain}/mobile/CheckChatWhereId.php?isAdd=true&idCus=$idCus&idStore=${customerRecordModel.id}';
