...
class ShowChat extends StatefulWidget {
  ...
  ShowChat({Key key, this.storeModel}) : super(key: key);

  @override
  _ShowChatState createState() => _ShowChatState();
}
...
class _ShowChatState extends State<ShowChat> {
  ...
  Future<Null> addChatThread() async {
    String url =
        '${MyConstant().domain}/mobile/addChat.php?isAdd=true&idCus=$idCus&idStore=${storeModel.id}';
    try {
  ...
  Future<Null> addChatDetailThread() async {
    String url =
        '${MyConstant().domain}/mobile/addChatDetail.php?isAdd=true&user_from=$usernameCus&user_to=${storeModel.username}&content=$content&chatId=$pro';
    try {
      Response response = await Dio().get(url);
      print('res = $response');
      if (response.toString() == 'true') {
        sendNotificaionToStore(content, usernameCus);
        print('คนรับ : ${storeModel.username}');
  ...
  Future<List<dynamic>> readDataChat(int value) async {
    ...
    String url =
        '${MyConstant().domain}/mobile/getChatDetail.php?isAdd=true&userCus=$usernameCus&userStore=${storeModel.username}';
    await Dio().get(url).then((value) {
  ...
  Future<Null> checkUser() async {
    ...
    setState(() {
      ...
      print('userTo = ${storeModel.username}');
    });
    String url =
        '${MyConstant().domain}/mobile/CheckChatWhereId.php?isAdd=true&idCus=$idCus&idStore=${storeModel.id}';
